    ...
    <div class="list_right">
      ...
      <ul class="list_bottm" >
        <li v-for="(item,index) in mesage" :key="index">
          ...
          @click="goNext('content',
          {'article_id':item.article_id,'parent_id':paramsO.parent_id,'cate_id':cate_id})">
...
<script>
    ...
    export default {
        ...
        methods: {
           ...
           GetNovice: function(callback) { //获取左侧分类列表
          
                let that = this;
                let val=this.paramsO.parent_id
        ...
        mounted() {
         let that= this;
          ...
          this.paramsO =  JSON.parse(decodeURIComponent(this.$route.params.orderyStatus));
          this.GetNovice(function(val){
           that.getcate(val[0].cate_id)
          })
          this.getcate(this.paramsO.cate_id)
