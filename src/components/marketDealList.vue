    ...
    <div class="main">
        <div class="tableWrap ">
            <el-button style=" height: 25px;
                 ...
                 margin-top: -41px;"  v-show="status == '1'"
            ...
            @click="togger">隐藏</el-button> -->
             <el-checkbox v-model="checked"  style=" height: 25px;
                line-height: 25px;   padding: 0 13px; float: right;
                margin-top: -41px; color:#4168f3" v-show="status == '3'">{{$t("Hide all canceled")}}</el-checkbox>
            <table class="table" width="100%" :class="status != '1' ?'topnone' : ''" >
                <thead v-if="status == '1'">
                <tr>
                    <th  style="text-align:left;" width="10%"><p>{{ $t("time") }}</p>　　</th>
                    <th  style="text-align:left;width:90px"> <p>{{ $t("market") }}</p></th>
                    <th width="10%"><p>{{ $t("type") }} </p></th>
                    <th  style="text-align:left;"> <p>{{ $t("direction") }}</p></th>
                    <th style="text-align:center;font-family: Helvetica;">{{$t("FIlled")}}</th>
                    <th  style="text-align:right;"><p>{{ $t("price") }}</p></th>
                    <!-- <th style="text-align:right;font-family: Helvetica;">成交数量</th> -->
                    <th style="text-align:right;width:170px"><p>{{ $t("amount") }}</p></th>
                    <!-- <th style="text-align:right;font-family: Helvetica;">成交金额</th> -->
                    <th style="text-align:right; width:170px"><p>{{ $t("total") }}</p></th>
                    <th  style="text-align:right;" v-show="status == '2'"><p>{{ $t("fee") }}</p></th>
                    <th  style="text-align:right;" width="10%"><p>{{
                        status==2 ?  $t("status") : $t("action")
                    }}</p></th>
                </tr>
                </thead>
                <tbody v-if="status == '1'" v-loading="loading">
                    <!-- //无数据状态 -->
                    <div :class="{none: enTrust != ''}"
                            ...
                            width: 1044px;
                            text-align: center;">
                        <p class=" " style="    color: #333333;font-weight:500">{{$t("no records")}}</p>
                    </div>
                <tr v-for="item in enTrust">
                    <td style="font-family: Helvetica;    padding: 15px 0px;" class="space1">{{dateTime(item.create_time).slice(5,20).replace("-","/")}}</td>
                    ...
                    <td style="text-align:right;font-family: Helvetica;" class="space1" :title="item.price">{{item.order_type == '1' ? format('0',item.price):$t('Market')}}</td>
                    <td style="text-align:right;font-family: Helvetica;" class="space1" id="colors" :title="item.count">{{item.order_type == '2' ?item.type == 1 ? '--':item.history_count : format('1',item.history_count)}}</td>
                    <td style="text-align:right;font-family: Helvetica;" class="space1" >
                        {{item.order_type == '2' ?item.type == 1 ? item.history_count:'--' : format('2',math_multiply(item.all_count))}}{{item.market}}
                    </td>
                    <td  style="text-align:right;">
                        <el-button style="padding: 0;font-size: 12px;color:#4168f3" @click="cancel(item.m_id)" type="text">{{statusText}}</el-button>
                    </td> 
                </tr>
                </tbody>
                <thead v-if="status == '2'">
                <tr>
                    <th  style="text-align:left;" width="20%"><p>{{ $t("time") }}</p>　　</th>
                    <th  style="text-align:left;"> <p>{{ $t("market") }}</p></th>
                    <th  style="text-align:left;width:80px"> <p>{{ $t("direction") }}</p></th>
                    <th  style="text-align:right;"><p>{{ $t("price") }}</p></th>
                    <th style="text-align:right;"><p>{{ $t("amount") }}</p></th>
                    <th style="text-align:right;"><p>{{ $t("total") }}</p></th>
                    <th  style="text-align:right;" v-show="status == '2'"><p>{{ $t("fee") }}</p></th>
                    <!-- <th  style="text-align:right;" width="10%"><p>{{
                        status==2 ?  $t("status") : $t("action")
                        }}</p></th> -->
                ...
                <tbody v-if="status == '2'">
                    ...
                    <div  :class="{none: dealList != ''}"
                      ...
                      <tr v-for="item in dealList">
                          <td class="space1" style="font-family: Helvetica;">{{dateTime(item.time_stamp)||'--'}}</td>
                          <td>{{item.coin}}/{{item.market}}</td>
                          <td  :class="item.type == 1 ?'green' : 'red'">{{item.type == 1 ? $t('buy') : $t('sell')}}</td>
                ...
                <tr>
                    ...
                    <th style="text-align:center;width:65px"> {{$t("action")}}</th>
                      ...
                      <tr v-for="item in dealList" :class="item.status==3&&a==0?'hiddenfirst':''">
                           <td style="font-family: Helvetica;" class="space1" >{{(formatDate(item.update_time).slice(5).slice(0,11))}}</td>
                        ...
                        <el-collapse v-show="item.status != '3'" accordion  v-model="activeName"   style="border-bottom:none;margin-top:0px;text-align:right width: 960px;margin-left: -960px;" >

                            <div  @click="getmesage(item.m_id,item.type)"> 

                            <el-collapse-item  style="text-align:right;"  :title="$t('details')" >
                                
                                    <table class="table tc" style="border-top:none;margin-top:10px;background: rgb(247,247,247);"  v-show="mesage.length > 0">
                                        <thead>
                                        <tr class="trs">
                                        ...
                                        <tbody>
                                        <tr v-for="item in mesage" class="trs">
                                           <!-- <td>21</td>
                                           <td>321</td> -->
