<template>
    <div class="rightCon">
        <div class="top">
            <p class="proName" style="padding-left:12px">{{$t("Income Records")}}</p>
        </div>
        <div class="tableWrap">
            <div class="bonus_m">
                <div class="bonus_m_t">
                ...
                <table class="table tc" style="border-top:none">
                    ...
                    <tr>
                        <th>{{ $t("time") }}</th>
                        <th style="text-align:right">{{$t("Coin")}}</th>
                        <th style="text-align:right; padding-right: 15px;">{{$t("Value")}}</th>
                    ...
                    <tr v-for="(val,index) in userBonusListD">
                        <td colspan="3">
                            <ul class="tab_ul">
                                ...
                                <li  style="text-align:right">{{val.project_coin}}</li>
                                <li style="text-align:right" v-if="val.income_detail.length >0"> 
                                    <el-collapse  accordion  v-model="activeName"   style="border-bottom:0;margin-top:0;text-align:right; width: 960px;margin-left: -634px;" >
                                        <el-collapse-item style="text-align:right;    padding-right: 0px;"  :title="lang=='zh' ?'¥'+ cionPriceA(val.income_detail).toFixed(2) : '$'+cionPriceA(val.income_detail).toFixed(2)" >
                                            <div class="bonus_m1">
                                                <table class="table tc" style="border-top:none">
                                                    <thead>
                                                    <tr>
                                                    ...
                                                    <tbody>
                                                    <tr v-for="item in val.income_detail">
                                    
                                                        <td>{{item.coin}}</td>
                                                        <td style="text-align:right">{{item.income}}</td>
                                                        <td  style="text-align:right;padding-right:20px">{{lang=='zh' ?'¥'+(cionPrice(item.coin,item.income) || 0).toFixed(2) : '$'+(cionPrice(item.coin,item.income) || 0).toFixed(2)}}</td>
                                                    </tr>
                                                    </tbody>
                                                </table>
                                            </div>
    ...
    export default {
        ...
        methods: {
            ...
            userBonusList: function () {//记录列表
                let that = this;
                gettokenuserincomedetail({page:this.page,perpage:this.pageSize}).then((data) => {
                    responseResult(data, (successData) => {
                        let info = successData.data;
            ...
            cionPriceA:function (val) {//获取总值
                ...
                cionL.forEach(function (val) {
                    ...
                    priceAll +=Number(that.cionPrice(val.coin,val.income)|| 0)
                });
                return priceAll
